...
abstract contract ERC20WithoutTotalSupply is IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if ((allowed >> 255) == 0) {
            _approve(sender, msg.sender, allowed.sub(amount));
        }
        return true;
    }
    ...
    function _burnFrom(address account, uint256 amount) internal {
        ...
        if ((allowed >> 255) == 0) {
            _approve(account, msg.sender, allowed.sub(amount));
        }
    }
}
...
contract ChiToken is IERC20, ERC20WithoutTotalSupply {
    ...
    uint8 constant public override decimals = 0;
    ...
    function freeFromUpTo(address from, uint256 value) public returns (uint256) {
        return freeFrom(from, min(min(value, balanceOf(from)), allowance(from, msg.sender)));
    }
