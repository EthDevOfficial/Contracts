...
library OptimizersReturn {
	using SafeMath for uint256;

	function getUniOptimalPreMP(
		OptimizerReturn.OptimalInfo memory params
		// bytes16 swapFee
	...
	function getAvgRootMP(
		uint256 reserve1A, 
		uint256 reserve1B,
		uint256 reserve2A,
		uint256 reserve2B
	) internal view returns (bytes16) {
		// may have problem with overflow
		// pools 1 and 2, coin types A and B
		uint256 rA = reserve1A.add(reserve2A);
		uint256 rB = reserve2A.add(reserve2B);
		bytes16 mp = ABDKFloat.div(ABDKFloat.fromUInt(rA), ABDKFloat.fromUInt(rB));
		return ABDKFloat.sqrt(mp);
	}
