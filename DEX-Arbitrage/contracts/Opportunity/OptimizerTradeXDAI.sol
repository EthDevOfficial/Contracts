...
contract OptimizerTradeXDAI {
	...
	constructor(address uniswapFactory, address payable exec) payable {
		...
		DXSWAP = 0xE43e60736b1cb4a75ad25240E2f9a62Bff65c0C0;
		DXSWAPROUTER = IDXSwapRouter(DXSWAP);
	}
	...
	function getSimpleTradeIterator(
		...
		out1in2 = params.exchange1 == DXSWAP ? DXSWAPROUTER.getAmountOut(input, awayInfo.reserveIn, awayInfo.reserveOut, 25) : IUniswapV2Router02(params.exchange1).getAmountOut(input, awayInfo.reserveIn, awayInfo.reserveOut);
		output = out1in2 > 0 ? (params.exchange2 == DXSWAP ? DXSWAPROUTER.getAmountOut(out1in2, returnInfo.reserveIn, returnInfo.reserveOut, 25) : IUniswapV2Router02(params.exchange2).getAmountOut(out1in2, returnInfo.reserveIn, returnInfo.reserveOut)) : 0;
		if (output == 0) {
			return (0, 0, 0);
		}
		...
		for (uint i = 0; i < MAX_ITERATIONS; i++) {
			...
			out1in2 = input > 0 ? (params.exchange1 == DXSWAP ? DXSWAPROUTER.getAmountOut(input, awayInfo.reserveIn, awayInfo.reserveOut, 25) :IUniswapV2Router02(params.exchange1).getAmountOut(input, awayInfo.reserveIn, awayInfo.reserveOut)) : 0;
			output = out1in2 > 0 ? (params.exchange2 == DXSWAP ? DXSWAPROUTER.getAmountOut(out1in2, returnInfo.reserveIn, returnInfo.reserveOut, 25) : IUniswapV2Router02(params.exchange2).getAmountOut(out1in2, returnInfo.reserveIn, returnInfo.reserveOut)) : 0;
			if (output == 0) {
				return (0, 0, 0);
			}
	...
	function getTriTradeIterator(
		...
		out1in2 = inputs.params.exchange1 == DXSWAP ? DXSWAPROUTER.getAmountOut(input, inputs.info1.reserveIn, inputs.info1.reserveOut, 25) : IUniswapV2Router02(inputs.params.exchange1).getAmountOut(input, inputs.info1.reserveIn, inputs.info1.reserveOut);
		out2in3 = out1in2 > 0 ? (inputs.params.exchange2 == DXSWAP ? DXSWAPROUTER.getAmountOut(out1in2, inputs.info2.reserveIn, inputs.info2.reserveOut, 25) : IUniswapV2Router02(inputs.params.exchange2).getAmountOut(out1in2, inputs.info2.reserveIn, inputs.info2.reserveOut)) : 0;
		output = out2in3 > 0 ? (inputs.params.exchange3 == DXSWAP ? DXSWAPROUTER.getAmountOut(out2in3, inputs.info3.reserveIn, inputs.info3.reserveOut, 25) : IUniswapV2Router02(inputs.params.exchange3).getAmountOut(out2in3, inputs.info3.reserveIn, inputs.info3.reserveOut)) : 0;
		if (output == 0) {
			return (0, 0, 0, 0);
		}
		...
		for (iter.iterator = 0; iter.iterator < MAX_ITERATIONS; iter.iterator++) {
			...
			out1in2 = input > 0 ? (inputs.params.exchange1 == DXSWAP ? DXSWAPROUTER.getAmountOut(input, inputs.info1.reserveIn, inputs.info1.reserveOut, 25) : IUniswapV2Router02(inputs.params.exchange1).getAmountOut(input, inputs.info1.reserveIn, inputs.info1.reserveOut)) : 0;
			out2in3 = out1in2 > 0 ? (inputs.params.exchange2 == DXSWAP ? DXSWAPROUTER.getAmountOut(out1in2, inputs.info2.reserveIn, inputs.info2.reserveOut, 25) : IUniswapV2Router02(inputs.params.exchange2).getAmountOut(out1in2, inputs.info2.reserveIn, inputs.info2.reserveOut)) : 0;
			output = out2in3 > 0 ? (inputs.params.exchange3 == DXSWAP ? DXSWAPROUTER.getAmountOut(out2in3, inputs.info3.reserveIn, inputs.info3.reserveOut, 25) : IUniswapV2Router02(inputs.params.exchange3).getAmountOut(out2in3, inputs.info3.reserveIn, inputs.info3.reserveOut)) : 0;
			if (output == 0) {
				return (0, 0, 0, 0);
	...
	function getQuadTradeIterator(
	...
	) internal view returns (uint256 input, uint256 out1in2, uint256 out2in3, uint256 out3in4, uint256 output) {
		input = inputs.info1.reserveIn > INPUT_STEP_DIVISOR ? inputs.info1.reserveIn / INPUT_STEP_DIVISOR : 1000;
		...
		out3in4 = out2in3 > 0 ? IUniswapV2Router02(inputs.params.exchange3).getAmountOut(out2in3, inputs.info3.reserveIn, inputs.info3.reserveOut) : 0;
		output = out3in4 > 0 ? IUniswapV2Router02(inputs.params.exchange4).getAmountOut(out3in4, inputs.info4.reserveIn, inputs.info4.reserveOut) : 0;
		if (output == 0) {
			return (0, 0, 0, 0, 0);
		}
		Iter memory iter = Iter(
			...
			getQuadImbalance(
				...
				inputs.info2.reserveOut - out2in3,
				inputs.info3.reserveIn + out2in3,
				inputs.info3.reserveOut - out3in4,
				inputs.info4.reserveIn + out3in4,
				inputs.info4.reserveOut - output
			) // post step imbalance
		);
		for (iter.iterator = 0; iter.iterator < MAX_ITERATIONS; iter.iterator++) {
			input = ABDKFloat.toUInt(
			...
			if (ABDKFloat.cmp(ITERATOR_THRESHOLD, ABDKFloat.abs(ABDKFloat.sub(iter.postStepImbalance, iter.targetImbalance))) == 1) {
				return (input, out1in2, out2in3, out3in4, output);
			}
		}
		return (input, out1in2, out2in3, out3in4, output);
	}
	...
	function getQuadOpportunity (
		...
		(info1.input, info1.output, info2.output, info3.output, info4.output) = getQuadTradeIterator(QuadIteratorInputs(params, imbalance, info1, info2, info3, info4));
		...
		return QuadOpportunity(
			...
			info1.input, info1.output, info2.output, info3.output, info4.output
		);
	}
